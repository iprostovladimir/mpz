...
export function formQuantity() {
  document.addEventListener("click", function (e) {
    ...
    if (targetElement.closest(".quantity__button")) {
      ...
      let minusButton = targetElement
        .closest(".quantity")
        .querySelector(".quantity__button_minus");
      if (value === 1) {
        minusButton.classList.add("_active");
      } else {
        minusButton.classList.remove("_active");
      }
    }
  });
}
...
// let form = document.querySelector("form");
// let inputs = form.querySelectorAll("input, select, textarea");
// let btn = document.querySelector(".js-btn-validate");
...
if (form) {
  inputs.forEach((input) => {
    input.addEventListener("input", () => {
      ...
      // Дополнительная проверка для поля form__input-phone с использованием Inputmask
      let phoneInput = form.querySelector(".form__input-phone");
      if (phoneInput && !phoneInput.inputmask.isComplete()) {
        errors++;
      }
